...
export function Select({
  ...
  return (
    <div ref={containerRef} className={cn('relative', widthClass)}>
      ...
      >
        <span className="truncate pr-6 flex items-center gap-2">
          {selectedOption && (selectedOption as any).color && (
            <div className={`w-3 h-3 rounded-full ${(selectedOption as any).color}`} />
          )}
          {displayValue}
        </span>
