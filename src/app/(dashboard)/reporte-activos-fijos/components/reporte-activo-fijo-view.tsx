...
import { FileText, User, Calendar, Package, AlertTriangle, CheckCircle, XCircle, Eye, Download, X, ChevronDown, ChevronUp } from 'lucide-react';

// Tipos
interface RecursoEvaluado {
  id_recurso: string;
...
interface ReporteActivoFijoViewProps {
  isOpen: boolean;
  onClose: () => void;
  reporte: ReporteActivoFijo | null;
}
...
const RecursoCard = ({ recurso, index }: { recurso: RecursoEvaluado; index: number }) => {
          ...
          {recurso.evidencia_urls && recurso.evidencia_urls.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
                {recurso.evidencia_urls.map((url, evidenciaIndex) => (
                  <ImageViewer
                    key={evidenciaIndex}
                    url={url}
...
export default function ReporteActivoFijoView({ isOpen, onClose, reporte }: ReporteActivoFijoViewProps) {
      ...
      <div className="sticky bottom-0 bg-[var(--card-bg)]/95 backdrop-blur-md border-t border-[var(--border-color)]/50 px-3 sm:px-6 py-3 sm:py-4 shadow-[0_-1px_3px_0_rgba(0,0,0,0.1),0_-1px_2px_0_rgba(0,0,0,0.06)]">
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          <div className="text-xs text-[var(--text-secondary)] hidden sm:block">
            {totalRecursos > 0 && `Mostrando ${totalRecursos} recurso${totalRecursos !== 1 ? 's' : ''}`}
          </div>
          <button
            onClick={onClose}
